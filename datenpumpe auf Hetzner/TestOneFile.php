 ...
 * Description   : SingleRecordTest.php
 * ----------------------------------------------------------------------------
 * Change Log    :
...
$log = dpLoghandler::CreateLogger('TestOneFile', 'run.log');
$fieldMapping2 = [
...
echo "Verbunden mit SugarCRM.\n";
$log->info("Verbunden mit SugarCRM");
//-----------------------------------------------------------------------------------------------------------
...
foreach ($files as $file) {
    ...
    if (file_exists($dir . '\\' . $file)) {
        ...
        //_____________________________________________________________
           //header mit sugar feldern umtauschen
        /*$HX = [];
        foreach ($header as $Hname) {
            $SugarDest = $fieldMapping[$Hname] ?? '';
            if (empty($SugarDest)) {
                $SugarDest = 'Ohne_Zielfeld_c';
            }
            $HX[] = $SugarDest;
        ...
        while (($data = fgetcsv($file, 0, ";")) !== FALSE) {
         ...
         //_____________________________________________________________
                   //leere felder raustun
           /* $row2 = array_combine($HX, $data);
            unset($row2['Ohne_Zielfeld_c'])   ;*/
         ...
         //_______________________________________________________________
            ...
            /*
                        ...
                        if (isset($get_entry_list_result['records']) && count($get_entry_list_result['records']) > 0) {
                            // Der Wert ist bereits vorhanden, überspringen Sie den Datensatz
                            echo "Datensatz mit CustNumb $custNumb wurde übersprungen.\n";
                            continue;
                        } else {
...
// Überprüfen, ob der Wert des "CustNumb" Headers bereits in SugarCRM vorhanden ist
            ...
            if (empty($custNumb) || !is_numeric($custNumb)) {
                $log->error('Da  Passt was nicht', ['accountData' => $accountData]);
            }
            $get_entry_list_result = $Client->GetAccounts($custNumb);
            if (isset($get_entry_list_result['records']) && count($get_entry_list_result['records']) > 0) {
                ...
                // Überprüfen, ob der Kontakt bereits vorhanden ist
                $contact = getContactByName($Client, $contactData['first_name'], $contactData['last_name']);
                if ($contact) {
                    ...
                    if (!empty($contactData['first_name']) && !empty($contactData['last_name'])) {
                        ...
                        echo "Erstelle neuen Kontakt mit Vorname 
                        {$contactData['first_name']} und Nachname {$contactData['last_name']}.\n";
                ...
                $KD_FromSugar = $result['kundennummer_c'];
                if ($KD_FromSugar != $custNumb) {
                    $log->error('Datensatzanlage  Passt nicht', ['csvrow' => $accountData, 'Sugar' => $result]);
                } else {
                    $log->debug('Datensatz Passt', ['csvrow' => $accountData, 'Sugar' => $result]);
                }
